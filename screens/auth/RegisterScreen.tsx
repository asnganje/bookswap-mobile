...
function RegisterScreen({ navigation }: Props) {
  ...
  const registrationSubmitHandler = async () => {
    const emailIsValid =
      userData.email.value.includes("@") && userData.email.value.includes(".");
    const nameIsValid = userData.fullname.value.trim().length > 0;
    const passwordIsValid = userData.password.value.length >= 6;
    const passwordsMatch =
      userData.password.value === userData.confirmPassword.value;
    ...
    <View style={styles.container}>
      ...
      <InputContainer
        ...
        onChangeText={(enteredText) =>
          inputChangeHandler("fullname", enteredText)
        }
      />
      <InputContainer
        placeholder="Email"
      ...
      <View style={styles.inputWrapper}>
        <InputContainer
          ...
          onChangeText={(enteredText) =>
            inputChangeHandler("password", enteredText)
          }
        />
        <TouchableButton
          onPress={passwordIsVisibleToggler}
      ...
      <View>
        <InputContainer
          ...
          onChangeText={(enteredtext) =>
            inputChangeHandler("confirmPassword", enteredtext)
          }
        />
        <TouchableButton
          onPress={confirmPasswordIsVisibleToggler}
          style={styles.eyeContainer}
        >
          <Ionicons
