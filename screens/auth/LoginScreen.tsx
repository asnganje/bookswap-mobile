...
function LoginScreen({navigation}: Props) {
  ...
  const loginValuesChangeHandler = (identifier: string, enteredText: string) => {
    setInputs((prev)=> ({
      ...prev, [identifier]: {value: enteredText, isValid: true}
    }))
  }

  const loginHandler = async () => {
    const emailIsValid = inputs.email.value.includes("@") && inputs.email.value.includes(".")
  ...
  return(
    <View style={styles.container}>
      ...
      <InputContainer
        ...
        onChangeText={(enteredText)=> loginValuesChangeHandler("email", enteredText)}
      ...
      <View style={styles.inputWrapper}>
        <InputContainer 
          ...
          onChangeText={(enteredText)=>loginValuesChangeHandler("password", enteredText)}
          placeholderTextColor={Styles.primary300}
        />
      ...
      <TouchableButton onPress={loginHandler}>
        {loading ? <LoadingUI color="#FFF"/>: <Text style={styles.buttonText}>Login</Text>}
      </TouchableButton>
